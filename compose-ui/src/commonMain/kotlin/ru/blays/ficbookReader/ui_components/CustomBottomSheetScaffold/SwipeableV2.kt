...
package ru.blays.ficbookReader.ui_components.CustomBottomSheetScaffold
...
internal class SwipeableV2State<T>(
    ...
    fun progressBetweenTwoValues(
        ...
        val absoluteOffset = offset ?: 0F
        val a = anchors[value1] ?: 0f
        val b = anchors[value2] ?: 0f
        val minValue = minOf(a, b)
        val maxValue = maxOf(a, b)
        val offsetBetweenValues = maxValue - minValue
        val offsetInRange = absoluteOffset - minValue
        val progress = (
            1 - (offsetInRange / offsetBetweenValues)
        ).coerceIn(0F..1F)
        return if (progress.isNaN()) 0F else progress
    }
